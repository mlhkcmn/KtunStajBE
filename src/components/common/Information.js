...
const Information = (props) => {
    ...
    const getAllPosts = async () => {
        const res = await axios.get('https://localhost:7050/api/Posts/2');
        const photo = await axios.get(`https://localhost:7050/api/Photos/${res.data.student_Photo_Id}`);
        await setTable(res.data);
        await setTable(existTable => ({
            ...existTable,
            photo: photo.data.photo_Code
        }));
    };
